...
object CompatibleSVGParser {
    ...
    private fun parseDocument(document: Document): SVG {
        ...
        loop@ for (element in svg.elements()) {
            val els =
                when (element.name) {
                    "circle" -> listOf(SVGCircle.fromElement(element))
                    "line" -> listOf(SVGLine.fromElement(element))
                    "polyline" -> listOf(SVGPolyline.fromElement(element))
                    "path" ->
                        try {
                            parsePath(element)
                    ...
                    "rect" -> listOf(SVGRect.fromElement(element))
                    else ->
                        throw UnsupportedOperationException(
                            "Element type ${element.name} is not supported!")
                }
    ...
    private fun parseTransform(attributeString: String): Transform {
        ...
        while (attributes.isNotEmpty()) {
            ...
            when (attributes.substring(0, firstOpenParen)) {
                "rotate" -> {
                    val parameters =
                        attributes.substring(firstOpenParen + 1, firstCloseParen).split(" ")
    ...
    private fun parseViewbox(attributeString: String?): Viewbox? {
        ...
        try {
            val splits =
                attributeString.replace(VIEWBOX_WHITESPACE, " ").split(" ").map { it.toFloat() }
            if (splits.size >= 3 && splits[2] <= 0f) return null
            if (splits.size >= 4 && splits[3] <= 0f) return null
