...
public class ScreenCfg extends GuiScreen {
    ...
    public ScreenCfg(IBrowser b, String vId) {
        browser = b;
        if (vId != null) b.loadURL("https://www.youtube.com/embed/" + vId + "?autoplay=1");
    ...
    public void handleInput() {
        ...
        while (Mouse.next()) {
            ...
            if (btn == 1 && pressed && sx >= x && sy >= y && sx < x + width && sy < y + height) {
                ...
                if (btn == 0 && !pressed) dragging = false;
                else {
                    x = sx + offsetX;
                    y = sy + offsetY;
                }
    ...
    public void drawScreen(int i1, int i2, float f) {
        ...
        if (drawSquare) {
            ...
            vb.pos(unscaleX(x + width), unscaleY(y + height), 0.0)
                    .color(255, 255, 255, 255)
                    .endVertex();
            vb.pos(unscaleX(x + width + 10), unscaleY(y + height), 0.0)
                    .color(255, 255, 255, 255)
                    .endVertex();
            vb.pos(unscaleX(x + width + 10), unscaleY(y + height + 10), 0.0)
                    .color(255, 255, 255, 255)
                    .endVertex();
            vb.pos(unscaleX(x + width), unscaleY(y + height + 10), 0.0)
                    .color(255, 255, 255, 255)
                    .endVertex();
            t.draw();
        }
