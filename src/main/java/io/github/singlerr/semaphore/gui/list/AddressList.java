...
public class AddressList extends ListScreenListBase<AddressListEntry> {
    ...
    private final PhoneScreen parent;

    private final int start;

    public AddressList(PhoneScreen parent,int start, int width, int height, int top, int size) {
    ...
    public void updateList(){
        Set<Map.Entry<UUID, State<?>>> states = CommonRegistries.getStatePool().getStates().stream().filter(e -> e.getValue() instanceof PlayerContext && ! e.getKey().equals(mc.getSession().getProfile().getId())).collect(Collectors.toSet());
