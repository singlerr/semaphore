 ...
 *
 */
@Mod(modid = "semaphore-mcef", dependencies = "required-after:mcef")
public class ExampleMod implements IDisplayHandler, IJSQueryHandler {

    public static ExampleMod INSTANCE;

    public ScreenCfg hudBrowser = null;
    private KeyBinding key = new KeyBinding("Open Browser 2", Keyboard.KEY_F10, "key.categories.misc");
    private Minecraft mc = Minecraft.getMinecraft();
    private ExampleBrowserScreen backup = null;
    private API api;
    ...
    public void setBackup(ExampleBrowserScreen bu) {
        backup = bu;
    }
    ...
    public IBrowser getBrowser() {
        if (mc.currentScreen instanceof ExampleBrowserScreen) return ((ExampleBrowserScreen) mc.currentScreen).browser;
        else if (backup != null) return backup.browser;
        else return null;
    }
    ...
    public void onTick(TickEvent ev) {
        if (ev.phase == TickEvent.Phase.START && ev.side == Side.CLIENT && ev.type == TickEvent.Type.CLIENT) {
            ...
            if (key.isPressed() && !(mc.currentScreen instanceof ExampleBrowserScreen)) {
                // Display the web browser UI.
                mc.displayGuiScreen(hasBackup() ? backup : new ExampleBrowserScreen());
                backup = null;
            }
        }
    }
    ...
    public void onAddressChange(IBrowser browser, String url) {
        ...
        if (mc.currentScreen instanceof ExampleBrowserScreen)
            ((ExampleBrowserScreen) mc.currentScreen).onUrlChanged(browser, url);
        else if (hasBackup()) backup.onUrlChanged(browser, url);
    }
    ...
    public void onDrawHUD(RenderGameOverlayEvent.Post ev) {
        if (hudBrowser != null) hudBrowser.drawScreen(0, 0, 0.f);
    }
}
