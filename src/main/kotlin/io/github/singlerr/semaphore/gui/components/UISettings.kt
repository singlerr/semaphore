...
class UISettings(
    ...
    init {
        ...
        volumeSettings = UIVolumes(this, container, ownerState, states)
    }

    fun removePlayerState(state: PlayerContext) {

        val children = volumeSettings.component.childrenOfType<UIVolume>()
        val child = children.find { c -> c.opponentId == state.owner }
        child?.let { volumeSettings.component.removeChild(it) }
    }

    fun addPlayerState(state: PlayerContext) {
        val children = volumeSettings.component.childrenOfType<UIVolume>()
        val child = children.find { c -> c.opponentId == state.owner }
        ...
        volumeSettings.component.addChild(UIVolume(ownerState, state.owner))
    }
