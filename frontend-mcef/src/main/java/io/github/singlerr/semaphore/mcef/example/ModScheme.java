...
public class ModScheme implements IScheme {
    ...
    public SchemePreResponse processRequest(String url) {
        ...
        int pos = url.indexOf('/');
        if (pos < 0) return SchemePreResponse.NOT_HANDLED;
        ...
        contentType = null;
        pos = loc.lastIndexOf('.');
        if (pos >= 0 && pos < loc.length() - 2) contentType = MCEF.PROXY.mimeTypeFromExtension(loc.substring(pos + 1));
    ...
    public void getResponseHeaders(ISchemeResponseHeaders rep) {
        if (contentType != null) rep.setMimeType(contentType);
